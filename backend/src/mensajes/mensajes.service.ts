...
export class MensajesService {
    ...
    async createMensaje(mensajeNuevo: CreateMensajeDto): Promise <Mensaje> {
        ...
        nuevo.nickname = mensajeNuevo.nickname;
    ...
    async updateMensaje(idMensaje: number, mensajeActualizar: CreateMensajeDto): Promise <Mensaje> {
        ...
        // 2. Verificamos si existe antes de editar
        if (!mensajeUpdate) {
            throw new Error('Mensaje no encontrado'); // O manejarlo en el controlador
        }
        mensajeUpdate.nickname = mensajeActualizar.nickname;
